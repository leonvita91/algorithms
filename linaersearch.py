...
def verfiy(index): # function takes one parameter as index variable
    if index is not None: # checking if index not return None 
        print(f'Target found in index {index}') # print found target and the index variable
    else:
...
# if you want to enable the first way just remove the #
# first way:
...
result = linear_search(newlist, 1) # as the first way we call linear_search func and we passing the newlist and target 
